...
> **版本**: v2.5.1 | **平台**: Windows 10 / 11 | **更新日期**: 2026-09-04
...
### 10.2.1 WorkBuddy 自动签到（v2.6.0 起）

- 同一卡片内的「WorkBuddy 自动签到」开关与时间设置，随顶部「保存」按钮生效
- **应用内定时器**：软件运行期间（含最小化到托盘）每日到点自动对「今日未签且无需重登」的 WorkBuddy 账号执行一键签到；软件未运行时不触发、不补签
- 迟启动补签：应用在设定时刻之后启动且当天尚未自动签到时，启动后约 30 秒内自动补执行一次
- 与手动签到互斥：自动触发时若手动签到正在进行，本轮顺延至下个周期；完成后 Toast 提示结果，顶栏账号状态自动刷新
- 执行状态记录于 `data/workbuddy_auto_checkin.json`（最近执行日期与结果），同一天最多自动执行一轮
